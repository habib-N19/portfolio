...
const BlogSection = () => {
  ...
  useGSAP(() => {
    xTo.current = gsap.quickTo(imageContainerRef.current, "left", { duration: 0.4, ease: "power3" });
    yTo.current = gsap.quickTo(imageContainerRef.current, "top", { duration: 0.4, ease: "power3" });
  ...
  useGSAP(() => {
    if (hoveredId) {
      gsap.to(imageContainerRef.current, { opacity: 1, scale: 1, duration: 0.25, ease: "power2.out" });
    } else {
      gsap.to(imageContainerRef.current, { opacity: 0, scale: 0.92, duration: 0.25, ease: "power2.in" });
    }
