...
const AboutSection = () => {
  ...
  // Handle Escape key to close resume
  useEffect(() => {
  ...
  // Lock scroll when resume panel is open
  useEffect(() => {
    if (isResumeOpen) {
        ...
        <div className="grid gap-16 lg:grid-cols-[1fr_1.2fr] lg:gap-24">
          ...
          <div>
            ...
            <div className="about-reveal mt-20 pt-16 border-t border-surface-border">
              <h3 className="font-mono-label text-text-secondary mb-8">JOURNEY / MILESTONES</h3>
              <div className="space-y-6">
                {timelineData.map((node) => (
                  <div key={node.year} className="grid grid-cols-[80px_1fr] items-start gap-4">
                    <div className="font-mono-data text-muted-foreground pt-1">{node.year}</div>
                    <div>
                      <h4 className="font-display text-2xl text-foreground tracking-wide leading-none">{node.title}</h4>
          ...
          >
            <div className="mx-auto max-w-3xl pb-24">
              ...
              <div className="mt-16 text-center">
                <h2 className="font-display text-5xl md:text-7xl">{resumeData.name}</h2>
              ...
              <div className="mt-16 space-y-16">
                ...
                <div className="flex justify-center gap-6">
                  <a href={`mailto:${resumeData.email}`} className="font-mono-data hover:underline">
                    {resumeData.email}
                  </a>
                  <a href={`https://${resumeData.site}`} target="_blank" rel="noopener noreferrer" className="font-mono-data hover:underline">
                    {resumeData.site}
                  </a>
                  <span className="font-mono-data text-muted-foreground">{resumeData.location}</span>
                </div>

                {/* Experience */}
                <div className="border-t border-black/10 pt-12">
                  <h3 className="font-mono-label mb-8 tracking-widest text-black/40">EXPERIENCE</h3>
                  <div className="space-y-12">
                    {resumeData.experience.map((exp: any, idx: number) => (
                      <div key={idx}>
                        <div className="flex flex-col md:flex-row md:items-baseline md:justify-between">
                          <h4 className="font-display text-2xl">{exp.role}</h4>
                          <span className="font-mono-data text-sm opacity-60">{exp.period}</span>
                        </div>
                        <p className="font-mono-data mt-1 text-sm opacity-80">{exp.company}</p>
                        <p className="font-editorial mt-4 leading-relaxed opacity-90">{exp.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Education */}
                <div className="border-t border-black/10 pt-12">
                  <h3 className="font-mono-label mb-8 tracking-widest text-black/40">EDUCATION</h3>
                  {resumeData.education.map((edu: any, idx: number) => (
                    <div key={idx} className="mb-6">
                      <div className="flex flex-col md:flex-row md:items-baseline md:justify-between">
                        <h4 className="font-display text-xl">{edu.degree}</h4>
                        <span className="font-mono-data text-sm opacity-60">{edu.year}</span>
                      </div>
                      <p className="font-mono-data mt-1 text-sm opacity-80">{edu.institution}</p>
                    </div>
                  ))}
                </div>
              </div>
