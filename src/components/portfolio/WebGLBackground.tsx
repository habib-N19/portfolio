  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < positionsAttr.count; i++) {
      ...
      if (wrapped) {
        ...
        positionsAttr.array[idxX] = currentX + (basePositions[idxX] - currentX) * lerpFactor
        positionsAttr.array[idxY] = currentY + (nextBaseY - currentY) * lerpFactor
        positionsAttr.array[idxZ] = currentZ + (basePositions[idxZ] - currentZ) * lerpFactor
      }
    }
