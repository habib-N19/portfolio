...
const TRAIL_IDS = ["trail-a", "trail-b", "trail-c", "trail-d", "trail-e"];

const CustomCursor = () => {
	const dotRef = useRef<HTMLDivElement>(null);
	const trailRefs = useRef<HTMLDivElement[]>([]);
	const [cursorState, setCursorState] = useState<
		"default" | "link" | "text" | "external"
	>("default");
	const mouse = useRef({ x: -100, y: -100 });
	const positions = useRef(
		Array.from({ length: TRAIL_IDS.length }, () => ({ x: -100, y: -100 })),
	);
	...
	useEffect(() => {
		...
		const onMouseOver = (e: MouseEvent) => {
			...
			let nextState: "default" | "link" | "text" | "external" = "default";
			if (el) {
				const cursorAttr = el.getAttribute("data-cursor");
				if (cursorAttr === "text") nextState = "text";
				else if (
